...
- **`allowed-tools` scopes the skill to `./context/**`** ([`SKILL.md`](../../skills/repo-business-context/SKILL.md) frontmatter), but the bootstrap flow offers to append a pointer to `CLAUDE.md`/`AGENTS.md` — a write outside that scope, which lands under the host agent's own permissions rather than the skill's. Worth re-checking whenever the frontmatter or the pointer offer changes. _(Inferred from the frontmatter and the bootstrap step, not from an observed failure.)_
